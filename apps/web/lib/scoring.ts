...
// Single source of truth for grape-to-flavor mapping.
// All 42 grapes from GRAPES constant are represented here.
export const GRAPE_FLAVOR_MAP: Record<string, FlavorProfile[]> = {
 ...
 * Returns a RecommendationScore with individual category scores and a matchPercentage.
 *
 * The wine argument uses Record<string, unknown> so both the recommendation engine
 * (which works with raw DB rows) and match-breakdown (which uses a typed subset)
 * can call this after casting.
 */
export function scoreWine(
  profile: WineProfileData,
  wine: Record<string, unknown>
): RecommendationScore & { matchPercentage: number } {
  ...
  return {
    ...
    priceScore,
    ratingScore,
    matchPercentage: Math.round((totalScore / MAX_SCORE) * 100),
  }
}
